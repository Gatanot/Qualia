 ...
 */
export const CURSOR_MARKER = "\x1b_qualia:c\x07";
...
export class TUI extends Container {
	...
	private showHardwareCursor = process.env.QUALIA_HARDWARE_CURSOR === "1";
	private clearOnShrink = process.env.QUALIA_CLEAR_ON_SHRINK === "1"; // Clear empty rows when content shrinks (default: off)
	...
	private doRender(): void {
		...
		const debugRedraw = process.env.QUALIA_DEBUG_REDRAW === "1";
		const logRedraw = (reason: string): void => {
			if (!debugRedraw) return;
			const logPath = path.join(os.homedir(), ".qualia", "agent", "qualia-debug.log");
		...
		// Configurable via setClearOnShrink() or QUALIA_CLEAR_ON_SHRINK=0 env var
		...
		for (let i = firstChanged; i <= renderEnd; i++) {
			...
			if (!isImage && visibleWidth(line) > width) {
				...
				const crashLogPath = path.join(os.homedir(), ".qualia", "agent", "qualia-crash.log");
				const crashData = [
		...
		if (process.env.QUALIA_TUI_DEBUG === "1") {
