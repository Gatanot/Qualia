...
三个 npm 包：`@gatanot/qualia_core`（引擎）、`@gatanot/qualia_web`（SvelteKit 应用）、`@gatanot/qualia`（用户入口，bin: qualia）。

**先本地测试，完成后再发布，防止版本号膨胀：**

```sh
# 1. 开发阶段：npm link 快速迭代（修改即生效，无需重新安装）
cd packages/cli && npm link
qualia serve                     # 直接从全局调用本地代码
...
**发布前检查清单**：
- 所有 `package.json` 的 `description` 不含 Unicode 特殊字符（PowerShell 会破坏编码）
- `@gatanot/qualia` 依赖版本号与实际发布的一致
- `vite.config.ts` 中 `ssr.external` 包含 `@gatanot/qualia_core`（防止 native 模块被打包）
